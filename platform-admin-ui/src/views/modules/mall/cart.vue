    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-tag v-if="scope.row.checked === 0" size="small" type="danger">否</el-tag>
          <el-tag v-else-if="scope.row.checked === 1" size="small" type="success">是</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="listPicUrl"
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button v-if="isAuth('mall:cart:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 查看详情
    showDetails (id, shopsName) {
      this.addOrUpdateVisible = true
      this.$nextTick(() => {
        this.$refs.addOrUpdate.init(id, shopsName, true)
      })
    },
    // 新增 / 修改
