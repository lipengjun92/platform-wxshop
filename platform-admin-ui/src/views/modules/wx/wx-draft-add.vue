  ...
  <el-dialog
    ...
    <el-row v-if="dataForm.articles.length>0">
      <el-col tag="center" :span="6">
        ...
        <el-button icon="el-icon-plus" @click="addDraftRow()" type="text">新建图文消息</el-button>
      </el-col>
      <el-col :span="18">
        <el-form :model="dataForm" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
                 label-width="80px">
          <el-tabs v-model="tableName">
            <el-tab-pane :label="item.title" v-for="(item, index) in dataForm.articles" :key="index" :name="index+''">
...
export default {
  ...
  methods: {
    ...
    onAssetsSelect (assetsInfo) {
      this.dataForm.articles[this.tableName].thumbUrl = assetsInfo.url
      this.dataForm.articles[this.tableName].thumbMediaId = assetsInfo.mediaId
