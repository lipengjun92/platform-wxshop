...
export default {
  ...
  methods: {
    ...
    _initEditor () {
      ...
      this.$refs.script.id = this.id = this.editorId || 'editor_' + Math.random().toString(16).slice(-6)
      this.init()
      this.$emit('before-init', this.id, this.mixedConfig)
      // 虽然这个驼峰的写法会导致使用 DOM 模版时出现监听事件自动转小写的 BUG，但如果经过编译的话并不会有这个问题，为了兼容历史版本，不做删除，参考 https://vuejs.org/v2/guide/components-custom-events.html#Event-Names
