...
export default {
	...
	methods: {
		...
		wxParseImgLoad(e) {
			...
			const styleStr = this.node.attr.width ? 'width:'+this.node.attr.width+'px' : '';
			const imageHeightStyle = mode === 'widthFix' ? '' : `height: ${imageheight}px;`;

			this.newStyleStr = `${imageHeightStyle}; width: ${imageWidth}px; padding: 0 ${+padding}px;${styleStr};`;//删除padding
